/* KAIRO v20.10.130 — Seller custom products + complete SVG logo pack (Blackbox excluded) */
(function(){
  'use strict';
  if(window.__KAIRO_SELLER_APP_PREMIUM_V129__) return;
  window.__KAIRO_SELLER_APP_PREMIUM_V129__ = true;
  ...
  function allEffective(){
    ...
    const baseKeys=new Set(CATALOG.map(rowKey));
  ...
  function txLocalDay(t){
    ...
    const raw=t?.reading_started_at||t?.created_at;if(!raw)return '';
  ...
  function cleanupSellerPerformance(){
    ...
    const plan=String((typeof activeWorkspacePlan!=='undefined'&&activeWorkspacePlan)||document.documentElement.dataset.workspacePlan||'basic').toLowerCase();
    const allowed=plan==='plus'||plan==='pro';
    document.body.classList.toggle('seller-performance-locked',!allowed);
    document.querySelectorAll('[data-tab="performance"],[data-mobile-tab="performance"]').forEach(el=>{
  ...
  function mount(){
    ...
    document.addEventListener('click',e=>{
      const perfTarget=e.target.closest('[data-tab="performance"],[data-mobile-tab="performance"]');
      if(perfTarget){
        const plan=String((typeof activeWorkspacePlan!=='undefined'&&activeWorkspacePlan)||document.documentElement.dataset.workspacePlan||'basic').toLowerCase();
      ...
      if(e.target.closest('[data-tab],[data-mobile-tab],.kairo-mobile-orders-main,#saas-settings-btn,#saas-settings-side-btn')){
        setTimeout(syncSellerTemplateChrome,40);setTimeout(syncSellerTemplateChrome,180);
      }
    ...
    syncSellerDashboardView();installSellerSettings();syncSellerTemplateChrome();
