    ...
    form.addEventListener('submit',async e=>{
      ...
      try{
        ...
        if(data?.session){
          ...
          status.textContent='Akun berhasil dibuat. Cek email untuk verifikasi, lalu masuk memakai username kamu.';
...
(function(){
 ...
 function openAccountPage(mode='signup'){buildPage();document.getElementById('kairo-account-page').classList.add('show');mode==='recover'?renderRecover():mode==='recovery-complete'?renderRecoveryComplete():renderSignup();window.scrollTo(0,0)}
 ...
 function hijackOldSignup(){const old=document.querySelector('.auth-mode-btn[data-auth-mode="signup"]');if(old){old.onclick=e=>{e.preventDefault();e.stopImmediatePropagation();openAccountPage('signup')};old.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();openAccountPage('signup')},true)}const oldForm=document.getElementById('signup-form');if(oldForm)oldForm.hidden=true}
...
(function(){
 ...
 function ensurePromoNav(){
   ...
   if(side){const order=['dashboard','input','promo','performance','customers','payout','cash'];order.forEach(k=>{const n=side.querySelector(`[data-tab="${k}"]`);if(n)side.appendChild(n)});const set=side.querySelector('#saas-settings-side-btn');if(set)side.appendChild(set)}
