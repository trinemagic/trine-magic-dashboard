...
(function(){
 ...
 function defaultDesign(){const cs=getComputedStyle(document.documentElement);return {background:'#FFFFFF',text:'#2D3748',accent:(cs.getPropertyValue('--brand-accent')||'#EA97A9').trim(),watermark:'',watermarkOpacity:8,watermarkSize:54,fontFamily:'Arial',description:'',descriptionAlign:'left',watermarkImage:'',showWorkspaceName:false,templateId:'pastel-commission'};}
 ...
 const RECEIPT_TEMPLATES=[
  ['pastel-commission','Pastel Commission','Playful pastel'],['studio-list','Studio List','Clean studio'],['receiptify','Receiptify','Thermal playful'],['vintage-story','Vintage Story','Vintage minimal'],
  ['newspaper-editorial','Newspaper Editorial','Editorial print'],['boarding-pass','Boarding Pass','Travel ticket'],['retro-diner','Retro Diner','Retro bold'],['minimal-luxury','Minimal Luxury','Premium minimal']
 ];
 function receiptTemplateId(d){const id=String(d?.templateId||'pastel-commission');return RECEIPT_TEMPLATES.some(x=>x[0]===id)?id:'pastel-commission';}
 function receiptTemplateName(id){return RECEIPT_TEMPLATES.find(x=>x[0]===id)?.[1]||'Pastel Commission';}
 function receiptBrandName(){return String(activeWorkspaceName||'Nama Usaha Anda').trim()||'Nama Usaha Anda';}
 function receiptPowered(){return `<div class="kairo-powered">powered by kairo workspaces</div>`;}
 function receiptBarcode(){return `<div class="kairo-barcode" aria-hidden="true"></div>`;}
 function templateShell(id,inner,accent){
   const brand=esc(receiptBrandName());
   const heads={
    'pastel-commission':`<div class="kairo-template-head pastel"><b>${brand}</b><em>for</em><strong>A KINDER TOMORROW</strong></div>`,
    'studio-list':`<div class="kairo-template-head studio"><b>${brand}</b><small>QUALITY MAKES A DIFFERENCE</small></div>`,
    'receiptify':`<div class="kairo-template-head receiptify"><b>${brand}</b><small>GOOD PEOPLE, GOOD PRODUCT</small></div>`,
    'vintage-story':`<div class="kairo-template-head vintage"><small>EST. 2024</small><b>${brand}</b><small>MORE THAN JUST A BUSINESS</small></div>`,
    'newspaper-editorial':`<div class="kairo-template-head newspaper"><strong>The Daily Order</strong><b>${brand}</b><small>SMALL BUSINESS, BIG IMPACT</small></div>`,
    'boarding-pass':`<div class="kairo-template-head boarding"><small>BUSINESS CLASS</small><b>${brand}</b><span>BOARDING TO A BRIGHTER TOMORROW</span></div>`,
    'retro-diner':`<div class="kairo-template-head diner"><small>GOOD PEOPLE · GOOD FOOD</small><b>${brand}</b><span>ALWAYS A GOOD CHOICE</span></div>`,
    'minimal-luxury':`<div class="kairo-template-head luxury"><small>EST. 2024</small><b>${brand}</b><span>BEAUTY IN EVERY DETAIL</span></div>`
   };
   return `<div class="kairo-receipt-template kairo-template-${esc(id)}" style="--kairo-template-accent:${esc(accent)}"><div class="kairo-paper">${heads[id]||heads['pastel-commission']}<div class="kairo-receipt-body">${inner}</div>${receiptBarcode()}${receiptPowered()}</div></div>`;
 }
 function buildHtml(p){const d=(layoutDirty?{...currentDesign(),...designDraft}:currentDesign()),bg=safeHex(d.background,'#FFFFFF'),tx=safeHex(d.text,'#2D3748'),ac=safeHex(d.accent,'#EA97A9'),wm=String(d.watermark||'').trim(),font=String(d.fontFamily||'Arial').replace(/["']/g,''),fontCss=receiptFontCss(font),description=String(d.description||'').trim(),descriptionAlign=['left','center','right','justify'].includes(String(d.descriptionAlign||''))?String(d.descriptionAlign):'left';const wmText=wm?`<div class="receipt-preview-watermark" style="color:${ac};opacity:${Math.max(0,Math.min(60,Number(d.watermarkOpacity||0)))/100};font-size:${Math.max(24,Math.min(180,Number(d.watermarkSize||54)))}px;font-family:${esc(fontCss)}">${esc(wm)}</div>`:'';const wmImg=d.watermarkImage?`<img src="${esc(d.watermarkImage)}" alt="Watermark" style="position:absolute;left:50%;top:50%;width:${Math.max(90,Math.min(360,Number(d.watermarkSize||54)*2.4))}px;max-height:70%;object-fit:contain;transform:translate(-50%,-50%) rotate(-24deg);opacity:${Math.max(0,Math.min(60,Number(d.watermarkOpacity||0)))/100};pointer-events:none;z-index:0">`:'';const receiptInner=`${wmText}${wmImg}<div class="receipt-customer-ready" style="position:relative;z-index:1;--receipt-accent:${ac};--receipt-font-family:${esc(fontCss)};font-family:${esc(fontCss)}">${description?`<div style="margin-bottom:14px;padding-bottom:12px;border-bottom:1px dashed ${ac}55;font-family:${esc(fontCss)};text-align:${descriptionAlign};line-height:1.55">${esc(description)}</div>`:''}${currentLayout().map(x=>itemHtml(x,p)).join('')}</div>`;const tid=receiptTemplateId(d);return `<div class="receipt-preview-surface" data-receipt-template="${esc(tid)}" style="position:relative;background:${bg};color:${tx};--receipt-font-family:${esc(fontCss)};font-family:${esc(fontCss)}">${templateShell(tid,receiptInner,ac)}</div>`;}
 ...
 function applyDesignControls(){const d=designDraft||currentDesign();const set=(id,v)=>{const e=document.getElementById(id);if(e)e.value=v};set('receipt-design-bg',d.background);set('receipt-design-text',d.text);set('receipt-design-accent',d.accent);set('receipt-design-watermark',d.watermark||'');set('receipt-design-watermark-opacity',d.watermarkOpacity??8);set('receipt-design-watermark-size',d.watermarkSize??54);set('receipt-design-font',d.fontFamily||'Arial');set('receipt-design-description',d.description||'');set('receipt-design-description-align',d.descriptionAlign||'left');updateColorSwatches();updateWatermarkPreview();refreshFontPicker(); const tpl=document.getElementById('receipt-template-select');if(tpl)tpl.value=receiptTemplateId(designDraft); }
 ...
 function rebuildSettings(){const settings=document.getElementById('settings'),select=document.getElementById('settings-category-select');if(!settings||!select)return;const old=settings.querySelector('[data-settings-panel="receipt"]');if(old)old.remove();let option=[...select.options].find(o=>o.value==='receipt');if(!option){option=document.createElement('option');option.value='receipt';option.textContent='Struk & Wording';select.appendChild(option)}
   ...
   panel.querySelector('#receipt-add-custom')?.addEventListener('click',()=>{layoutDirty=true;openEditor(`custom-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,true)});panel.querySelector('#receipt-layout-save')?.addEventListener('click',saveLayout);panel.querySelector('#receipt-template-select')?.addEventListener('change',e=>{designDraft.templateId=e.target.value;layoutDirty=true;panel.querySelectorAll('[data-receipt-template-choice]').forEach(b=>b.classList.toggle('active',b.dataset.receiptTemplateChoice===e.target.value));});panel.querySelectorAll('[data-receipt-template-choice]').forEach(b=>b.addEventListener('click',()=>{const sel=panel.querySelector('#receipt-template-select');if(sel){sel.value=b.dataset.receiptTemplateChoice;sel.dispatchEvent(new Event('change',{bubbles:true}));}panel.querySelector('#receipt-design-preview-btn')?.click();}));setTimeout(()=>{const sel=panel.querySelector('#receipt-template-select');panel.querySelectorAll('[data-receipt-template-choice]').forEach(b=>b.classList.toggle('active',b.dataset.receiptTemplateChoice===(sel?.value||'pastel-commission')))},0);
